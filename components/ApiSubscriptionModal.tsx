...
const ApiSubscriptionModal: React.FC<ApiSubscriptionModalProps> = ({ onClose }) => {
    ...
    const renderContent = () => {
        switch (view) {
            case 'success': return (<div className="text-center p-8"><CheckIcon className="mx-auto h-16 w-16 text-green-500 animate-check-pop" /><h2 className="text-3xl font-bold text-white mt-4">Upgrade Successful!</h2><p className="text-gray-400 my-4">Your API key has been upgraded.</p><button onClick={onClose} className="w-full max-w-xs mx-auto rounded-md bg-brand-primary px-3 py-3 text-sm font-semibold text-brand-dark">Close</button></div>);
