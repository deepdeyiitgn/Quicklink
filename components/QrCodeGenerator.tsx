...
import {
    ...
    CalendarIcon, BitcoinIcon, UpiIcon, ImageIcon, ColorPaletteIcon, ChevronDownIcon, WhatsappIcon,
    PayPalIcon, SkypeIcon, ZoomIcon
...
const defaultQrData: Record<QrCodeType, any> = {
    url: { url: 'https://qlynk.vercel.app/' },
    ...
    paypal: { email: '', amount: '', currency: 'USD', item: '' },
    skype: { username: '', action: 'chat' },
...
const QrCodeGenerator: React.FC = () => {
    ...
    const getQrString = (): string => {
        switch(activeType) {
            ...
            case 'whatsapp': return `https://wa.me/${qrData.phone}?text=${encodeURIComponent(qrData.message || '')}`;
            case 'paypal': return `https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=${encodeURIComponent(qrData.email || '')}&item_name=${encodeURIComponent(qrData.item || '')}&amount=${encodeURIComponent(qrData.amount || '')}&currency_code=${qrData.currency || 'USD'}`;
            case 'skype': return `skype:${qrData.username || ''}?${qrData.action || 'chat'}`;
            case 'zoom': return `https://zoom.us/j/${qrData.meetingId || ''}${qrData.password ? `?pwd=${qrData.password}` : ''}`;
            default: return '';
        }
    };
    ...
    const renderForm = () => {
        switch(activeType) {
            ...
            case 'whatsapp': return (<div className="space-y-4"><FormInput label="Phone Number (with country code)" type="tel" name="phone" value={qrData.phone || ''} onChange={handleInputChange} placeholder="919876543210" /><FormTextarea label="Message (optional)" name="message" value={qrData.message || ''} onChange={handleInputChange} rows={3} /></div>);
            case 'paypal': return (<div className="space-y-4"><FormInput label="PayPal Email" type="email" name="email" value={qrData.email || ''} onChange={handleInputChange} /><FormInput label="Item Name" type="text" name="item" value={qrData.item || ''} onChange={handleInputChange} /><FormInput label="Amount" type="number" name="amount" value={qrData.amount || ''} onChange={handleInputChange} step="any" /><FormInput label="Currency" type="text" name="currency" value={qrData.currency || 'USD'} onChange={handleInputChange} /></div>);
            case 'skype': return (<div className="space-y-4"><FormInput label="Skype Username" type="text" name="username" value={qrData.username || ''} onChange={handleInputChange} /><FormSelect label="Action" name="action" value={qrData.action || 'chat'} onChange={handleInputChange}><option value="chat">Chat</option><option value="call">Call</option><option value="voicemail">Voicemail</option></FormSelect></div>);
            case 'zoom': return (<div className="space-y-4"><FormInput label="Zoom Meeting ID" type="text" name="meetingId" value={qrData.meetingId || ''} onChange={handleInputChange} /><FormInput label="Password (optional)" type="text" name="password" value={qrData.password || ''} onChange={handleInputChange} /></div>);
            default: return null;
        }
    }
